...
int main(int argc, char *argv[]) {
	...
	DIC_3D_Input dic_in("3D_DIC_input.txt");
	dic_in.debug_print(std::cout);

    std::array<size_t,2> image_xy_min{0 , 0}, image_xy_max{2048, 2048};
    int stack_h = 106;
    std::string image_format = ".jpg";
    double coef_treshold = 0.15;

    size_t ref_image_stack_number = 2;
    size_t def_image_stack_number = 3;
    ...
    size_t s_z = stack_h / dic_in.z_bounce + 1;

    size_t subset_number = s_x * s_y;
    std::vector<int> interesting_layers = range(0, stack_h, dic_in.z_bounce);
    std::vector<std::vector<int>> result_ref(s_z * subset_number, std::vector<int>(3, 0));
    auto result_def = result_ref;
    std::vector<double> result_coefs(s_z * subset_number, size_t(-1));
    ...
    for (size_t i = 0; i < s_y; ++i) {
        for (size_t p = 0; p < s_x; ++p) {
            ...
            for (auto k : interesting_layers) {
                ...
                auto ref_image_str = dic_in.images_folder + dic_in.image_name_prefix + std::to_string(ref_image_stack_number) + dic_in.image_name_postfix + ref_z_str.str() + image_format;
                ...
                if (search_z_max >= stack_h)
                    search_z_max = stack_h - 1;

                double best_coef = 1e6;
                ...
                for (int z = init_z; z < search_z_max + 1; ++z) {
                    ...
                    auto def_image_str = dic_in.images_folder + dic_in.image_name_prefix + std::to_string(def_image_stack_number) + dic_in.image_name_postfix + def_z_str.str() + image_format;
                    ...
                    else if (best_coef < coef_treshold){
                        break; 
                    }
                ...
                if ((best_z > init_z || (init_z - 1) < 0) & (best_coef < coef_treshold)) {
                    result_def[xyz_table_start + k_bounced] = {subset_center[0] + best_u, subset_center[1] + best_v, best_z}; // double to int !!
                ...
                for (int z = init_z - 1; z >= search_z_min + 1; --z) {
                    std::stringstream def_z_str;
                    def_z_str << std::setw(3) << std::setfill('0') << std::setw(3) << z;
                    auto def_image_str = dic_in.images_folder + dic_in.image_name_prefix + std::to_string(ref_image_stack_number) + dic_in.image_name_postfix + def_z_str.str() + image_format;
                    auto res = CalculateSubsetDisp(dic_in, ref_image_str, def_image_str, s_xy_min);
